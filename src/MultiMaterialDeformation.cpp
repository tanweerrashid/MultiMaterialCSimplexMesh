...
#include "MultiMaterialDeformation.h"

MultiMaterialDeformation::MultiMaterialDeformation(CSimplexSurf **newMesh, double timestep, double alpha, int num_meshes) {
	meshes = newMesh;
	...
	numberOfDeformableMeshes = num_meshes;
}

MultiMaterialDeformation::~MultiMaterialDeformation() {
}

void MultiMaterialDeformation::setMeshes(CSimplexSurf **newMesh) {
	meshes = newMesh;
}

CSimplexSurf** MultiMaterialDeformation::getMeshes() {
	return meshes;
}

P_float MultiMaterialDeformation::getTimestep() {
	return Timestep;
}

void MultiMaterialDeformation::setTimestep(double ts) {
	Timestep = ts;
	Timestep2 = Timestep * Timestep;
}

P_float MultiMaterialDeformation::getAlpha() {
	return Alpha;
}

void MultiMaterialDeformation::setAlpha(double a) {
	Alpha = a;
	Alpha2 = Alpha * Alpha;
}

int MultiMaterialDeformation::getNumberOfDeformableMeshes() {
	return numberOfDeformableMeshes;
}

void MultiMaterialDeformation::setNumberOfDeformableMeshes(int n) {
	numberOfDeformableMeshes = n;
}

void MultiMaterialDeformation::ImpEuler() {
...
void MultiMaterialDeformation::ImpEuler_ApplyResults(P_float* x) {
...
void MultiMaterialDeformation::ImpEuler_SetParams(P_float* r, P_float* h) {
